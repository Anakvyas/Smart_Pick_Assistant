...
def compute_hint(brightness, sharpness, codes, w, h, has_label):
    ...
    Returns (kind, text): `kind` lets a caller (here, and the frontend) tell
    a genuine "nothing recognisable in this frame" ("no_product") apart from
    a merely fixable camera-quality issue (dark/blur/too close/too far) —
    the same distinction the YOLO product gate makes when it's enabled, kept
    available even when that model isn't installed (see USE_YOLO_GATE),
    since the frontend's "NO PRODUCT" banner needs it either way.
    """
    if brightness < 60:
        return "dark", "💡 too dark — move to better light"
    if sharpness < 60:
        return "blur", "🫳 hold the camera steady"
    if codes:
        areas = [((c["box"][2] - c["box"][0]) * (c["box"][3] - c["box"][1])) / (w * h)
                  for c in codes]
        max_area = max(areas)
        if max_area < 0.01:
            return "closer", "🔍 move closer to the product"
        if max_area > 0.9:
            return "back", "↔️ move back a little"
        return None, None
    if not has_label:
        return "no_product", "📷 point the camera at a product label"
    return None, None
...
def analyze_image(img_bytes, want_image=False, allow_llm=True, img=None):
    ...
    if USE_YOLO_GATE:
        ...
        if not product["is_product"]:
            return {"valid": False, "message": "empty shelf — no product detected",
                    "codes": [], "barcode": None, "barcode_type": None,
                    **ocr_service.label_payload(None),
                    "hint": "🛒 empty shelf — point the camera at a product",
                    "hint_kind": "no_product",
    ...
    codes = [{"kind": c["kind"], "value": c["value"],
             ...
             for c in codes_raw]
    # Flat convenience fields alongside `codes` — most callers just want
    # "the" barcode on a single-product photo, not to reach into the list.
    barcode = codes_raw[0]["value"] if codes_raw else None
    barcode_type = codes_raw[0]["kind"] if codes_raw else None

    hint_kind, hint = compute_hint(brightness, sharpness, codes_raw, w, h, not ocr_service.label_empty(label))
    ...
    return {"valid": valid, "message": message, "codes": codes,
            ...
            **ocr_service.label_payload(label), "hint": hint, "hint_kind": hint_kind,
