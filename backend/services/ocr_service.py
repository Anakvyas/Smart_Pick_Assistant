...
# Manufacturing/packing date — same date-shape ambiguity as expiry, so it's
# just as keyword-gated. Requires "date"/"dt"/"on" (or one of the standalone
# abbreviations MFD/DOM/PKD) rather than bare "MFG"/"MFD" alone, so this
# never fires on "Mfg by ACME Foods" (a company line, not a date) — that
# phrase is "MFG" immediately followed by "by", which none of these
# alternatives match. Unlike expiry, there's no bare-text fallback below:
# a label that never prints an explicit mfg-date keyword should report no
# mfg date rather than guess at some other date on the pack (most likely
# the real expiry, which would then wrongly show up twice).
MFG_KEYWORDS = re.compile(
    r"\b(MFG\.?\s*D(?:ATE|T)\.?|MFD\.?|DATE\s*OF\s*MANUFACTURE|"
    r"MANUFACTURING\s*DATE|MANUFACTURED\s*ON|DOM\.?|PKD\.?|PACKED\s*ON|"
    r"PACKAGING\s*DATE)", re.I)
...
_EMPTY_LABEL = {"name": None, "mfg_date": None, "expiry": None, "gstin": None,
                "weight": None, "mrp": None, "company": None, "address": None,
                "batch": None, "raw_text": None}
...
_NAME_EXCLUDE_RE = re.compile(
    ...
    r"BATCH|LOT\s*NO\.?|B\.?NO\.?|MFG\w*|MFD|DOM|PKD|MANUFACTURED|MARKETED|MKTD\w*|ADDRESS)", re.I)
...
def read_label(img, allow_llm=True):
    ...
    mfg_date = None
    for km in MFG_KEYWORDS.finditer(text):
        mfg_date = _parse_date(text[km.end():km.end() + 20], default)
        if mfg_date:
            break
    ...
    name = _largest_text_line(
        lines, exclude_substrings=[gstin, weight, mrp_val, company, address, batch, expiry, mfg_date])
    ...
    if llm_service.llm_enabled and allow_llm:
        ...
        if filled:
            ...
            llm_mfg = _parse_date(str(filled.get("mfg_date") or ""), default)
            mfg_date = llm_mfg or mfg_date

    label = {"name": name, "mfg_date": mfg_date, "expiry": expiry, "gstin": gstin,
             "weight": weight, "mrp": mrp, "company": company, "address": address,
             "batch": batch, "raw_text": text}
